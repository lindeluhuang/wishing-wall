  ...
  return (
    <Card className="wishItem">
      <CardBody>
        ...
        <CardSubtitle>
          <img src={heartIcon} className="heart-icon" onClick={handleClick} />
          {heartsToDisplay} hearts • {wish.datedisplay}
        </CardSubtitle>
      </CardBody>
    </Card>
  );
...
function WishesList({wishes, hearts, plusHeart, isLoading, errMess}) {
  ...
  if (wishes) {
    ...
    return (
      <div className="container">
        <div className="card-columns">{getWishes}</div>
      </div>
    );
  }
}
